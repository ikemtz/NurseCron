...
export app1Name=$envLower"-wa-empo-nrcrn"
...
# Service specific
export entityName="Employee"
export swaggerClientId="6veGdlIerXfNDn6bHYHNu97jgpi4FzY5"
export swaggerAudience=$envUpper"-EmpO"
export validAudiences="$envUpper-NurseCron,$swaggerAudience"
export ainName=$envLower"-ai-core-nrcrn"
export dockerImageName="ikemtz/nurse-cron-employees:odata_latest"
...
if [ -z $connectionString ]; then
    ...
    sqlcmd -S $sqlSrvName.database.windows.net -U $sqlAdminUser -P $sqlAdminPass -d "$sqlDbName" -Q "$addRoleQuery"
